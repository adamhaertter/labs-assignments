/* CSC111 Computer Science II
   Lab 7 Reminder List Pointer - remindv3.c
   Programmers: Adam Haertter & Tyler Owens
   Professor: Dr. Lee
   File Created: Oct 13, 2019
   File Updated: Oct 16, 2019
*/
...
int main()
{
   ...
   for(;;){
	...
	for(i = 0; i < num_remind; i++)
	   if(strncmp(day_str, p, 3) < 0)			//Compares the first 3 digits of day_str and reminders[i]
		break;						//Breaks nested loop if the day in day_str < day in reminders
	p = &reminders[j][0];
	for(j = num_remind; j > i; j--)
	   strncpy((p-j), (p-(j-1)), MSG_LEN+3);		//Shifts the reminders up one in the array if less than MSG_LEN+3

	p = &reminders[i][0];
	strncpy(p, day_str, 3);					//Copies the day in day_str to reminders if under 3 chars
	strncat(p, msg_str, MSG_LEN+1);				//Adds the message to the end of the day text if under MSG_LEN+1
