...
int main()
{
   ...
   for(;;){
	...
	for(j = num_remind; j > i; j--)
	   strcpy(reminders[j], reminders[j-1]);	//Shifts the reminders up one in the array

	strcpy(reminders[i], day_str);			//Copies the day in day_str to reminders
	strcat(reminders[i], msg_str);			//Adds the message to the end of the day message in reminders
